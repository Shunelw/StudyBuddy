...
const CourseView = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setProgress({ completed: 0, total: (course?.lessons || []).length, percentage: 0 });
      }
    })();
    return () => { cancelled = true; };
  }, [user?.id, course?.id]);
  ...
  if (!course) {
    return (
      <div className="container" style={{ padding: '4rem 0', textAlign: 'center' }}>
        ...
        <button onClick={() => navigate('/student/dashboard')} className="btn btn-primary">Browse Courses</button>
      </div>
    );
  }
  ...
  const handleEnroll = async () => {
    if (course.price !== 0 && !window.confirm(`Enroll in "${course.title}" for $${course.price}?`)) return;
    try {
      await api.enrollments.enroll(user.id, course.id);
      updateUser({ enrolledCourses: [...(user.enrolledCourses || []), course.id] });
  ...
  const handleCompleteLesson = async (lessonId) => {
    ...
    try {
      await api.progress.completeLesson(user.id, lessonId);
      updateUser({ completedLessons: [...completedLessonIds, lessonId] });
      setProgress((prev) => {
        const total = lessons.length;
        const completed = prev.completed + 1;
        return { ...prev, completed, total, percentage: total ? Math.round((completed / total) * 100) : 0 };
      });
    } catch (_) {}
  };
          ...
          <div className="course-header-content">
            <div className="course-header-left">
              ...
              <div className="course-meta">
                <div className="meta-item"><Star size={18} fill="currentColor" /><span>{course.rating}</span></div>
                <div className="meta-item"><Users size={18} /><span>{course.students} students</span></div>
                <div className="meta-item"><Clock size={18} /><span>{course.duration}</span></div>
                <div className="meta-item"><BookOpen size={18} /><span>{lessons.length} lessons</span></div>
              </div>
              ...
              {!isEnrolled ? (
                <div className="enroll-card">
                  <div className="course-price">
                    {course.price === 0 ? <span className="free-price">FREE</span> : <span className="paid-price">${course.price}</span>}
                  </div>
                  <button onClick={handleEnroll} className="btn btn-primary btn-lg btn-block">Enroll Now</button>
                  <div className="enroll-info"><CheckCircle size={16} /><span>Full lifetime access</span></div>
                </div>
              ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="lesson-info">
                      ...
                      <div className="lesson-meta"><Clock size={14} /><span>{lesson.duration}</span></div>
                    </div>
                    <div className="lesson-status">
                      {isLocked ? <Lock size={20} /> : isCompleted ? <CheckCircle size={20} className="completed-icon" /> : <Play size={20} />}
                    </div>
                  </div>
                );
              })}
            ...
            {selectedLesson ? (
              <div className="lesson-viewer animate-fade-in">
                ...
                <div className="lesson-content">
                  ...
                  <div className="content-text">{selectedLesson.content}</div>
                  <div className="lesson-materials">
                    <h4>Materials</h4>
                    <button className="material-btn"><Download size={18} /> Download Lesson Notes.pdf</button>
                  </div>
                </div>
                <div className="lesson-actions">
                  {!completedLessonIds.includes(selectedLesson.id) && (
                    <button onClick={() => handleCompleteLesson(selectedLesson.id)} className="btn btn-primary">
